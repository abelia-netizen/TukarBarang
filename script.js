...
function renderHomeGrid() { document.getElementById('home-grid').innerHTML = items.slice(0, 6).map(createItemCard).join(''); }
function renderCatalogGrid(filter = 'all') {
  ...
  grid.innerHTML = filtered.length === 0 ? `<div class="empty-state">${svgIcon('box', 48, 48, 'stroke:var(--muted);opacity:0.3')}<p>Belum ada barang</p></div>` : filtered.map(createItemCard).join('');
}
...
function openModal(id) {
  ...
  const badgeClass = { free: 'badge-free', swap: 'badge-swap', sell: 'badge-sell' };
  const statusIcon = { free: 'gift', swap: 'swap', sell: 'tag' };
  document.getElementById('modal-img').innerHTML = item.photoUrl ? `<img src="${item.photoUrl}" alt="${item.name}"/>` : svgIcon('image', 80, 80, 'stroke:var(--muted);opacity:0.4');
  document.getElementById('modal-meta').innerHTML = `<div class="item-badge ${badgeClass[item.status]}">${svgIcon(statusIcon[item.status], 11, 11)} ${statusMap[item.status]}${item.status === 'sell' && item.harga ? ' Rp '+Number(item.harga).toLocaleString('id-ID') : ''}</div><div class="item-condition">${item.kondisi}</div>`;
  document.getElementById('modal-title').textContent = item.name;
  document.getElementById('modal-desc').textContent = item.desc || 'Tidak ada deskripsi';
  document.getElementById('modal-owner').innerHTML = `<div class="owner-avatar">${item.ownerInitial}</div><div><div class="owner-name">${item.owner}</div><div class="owner-rating modal-stars">${starIcons(item.rating)} ${item.rating}</div></div>`;
...
function doSearch() {
  ...
  const grid = document.getElementById('catalog-grid');
  grid.innerHTML = results.length === 0 ? `<div class="empty-state">${svgIcon('search', 48, 48, 'stroke:var(--muted);opacity:0.3')}<p>Tidak ada barang cocok</p></div>` : results.map(createItemCard).join('');
}
...
function handlePhotoUpload(event) {
  ...
  for(let i=0; i<files.length; i++) {
    ...
    reader.onload = function(e) { uploadedPhotos.push(e.target.result); renderPhotoPreview(); }
...
function renderPhotoPreview() {
  ...
  preview.innerHTML = '';
  uploadedPhotos.forEach((src, index) => { preview.innerHTML += `<div class="photo-thumb-wrap"><img src="${src}" alt="Foto ${index+1}"/><button class="photo-remove" onclick="removePhoto(${index})">✕</button></div>`; });
}
...
function validateWhatsApp(num) {
  ...
  if (clean.startsWith('8') && clean.length >= 9) clean = '62' + clean;
  if (!/^62\d{8,12}$/.test(clean)) return null;
  return clean;
}
...
function submitForm() {
  ...
  const waInput = document.getElementById('f-wa').value.trim();
  const kec = document.getElementById('f-kecamatan').value;
  ...
  const waNumber = validateWhatsApp(waInput);

  if(!nama || !kategori || !kondisi || !selectedStatus || !kec || !desa) { 
    showToast('Harap isi field wajib (*)', 'warning'); return; 
  }
  if(!waInput) { 
    showToast('Nomor WhatsApp wajib diisi', 'warning'); return; 
  }
  if(!waNumber) { 
    showToast('Nomor WhatsApp tidak valid. Contoh: 81234567890', 'warning'); return; 
  }

  const newItem = {
    id: items.length + 1, name: nama, kategori: kategori, kondisi: kondisi, desc: desc,
    ...
    photoUrl: uploadedPhotos[0] || '', wa: waNumber
  };
  ...
  document.getElementById('f-desa').value = ''; document.getElementById('f-dusun').value = ''; document.getElementById('f-rtrw').value = '';
  selectedStatus = ''; document.querySelectorAll('.status-opt').forEach(opt => opt.classList.remove('selected'));
...
function previewProfilePhoto(event) {
  ...
  if(file) {
    ...
    reader.onload = function(e) {
      ...
      document.getElementById('edit-photo-preview').style.backgroundImage = `url(${e.target.result})`;
      document.getElementById('edit-photo-preview').style.backgroundSize = 'cover';
    }
...
function saveProfile() {
  ...
  document.getElementById('profile-loc-display').innerHTML = `<svg><use href="#ic-pin"/></svg> ${userProfile.location} · Bergabung Januari 2024`;
  
  if(userProfile.photo) {
    document.getElementById('profile-avatar-display').innerHTML = `<img src="${userProfile.photo}" style="width:100%;height:100%;object-fit:cover;"/>`;
  } else {
    document.getElementById('profile-avatar-display').innerText = userProfile.initial;
  }

  items.forEach(item => {
    if(item.ownerInitial === userProfile.initial || item.owner === "Budi Santoso") {
...
document.addEventListener('DOMContentLoaded', () => {
  initProvinces();
  renderHomeGrid(); renderCatalogGrid(); renderProfileGrid(); renderNotifications();
});